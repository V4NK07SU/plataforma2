...
(function() {
  "use strict";
  angular
    .module("app.modules.agenda.appointments")
    .controller("AgendaAppointmentsCtrl", [
      "$scope",
      "$window",
    ...
    .controller("AgendaAppointmentsIndexCtrl", [
      ...
      "moment",
      "amMoment",
      "AgendaAppointmentSrv",
      "ToastService",
      "DialogService",
      "$http",
      AgendaAppointmentsIndexCtrl
    ])
    .controller("AgendaAppointmentsShowCtrl", [
      "$scope",
      "$window",
    ...
    .controller("AgendaAppointmentsFormCtrl", [
      "$scope",
      "$window",
  ...
  /**
     ...
     * @constructor
     */
  function AgendaAppointmentsIndexCtrl(
    $scope,
    $window,
    $state,
    moment,
    ...
    $scope.search = function(keyword) {
      ...
      if (keyword) {
        $http
          .get(SITE_URL + "/api/agendas/appointments/search/" + keyword)
          .success(function(res) {
            $scope.data = res;
            $scope.keyword = "";
          })
    ...
    $scope.deleteAppointments = function(appointments) {
      ...
      DialogService.confirm("Eliminar citas", "Desea continuar?").then(() => {
        AgendaAppointmentSrv.delete(
          { id: appointments.id },
          function(response) {
  ...
  /**
     ...
     * @constructor
     */
  function AgendaAppointmentsCreateCtrl(
    $scope,
    $window,
    moment,
    AgendaAppointmentSrv,
    ...
    $scope.formUrl =
      THEME_URL + "/app/modules/agenda/appointments/views/form.html";

    $scope.appointments = {};

    console.log($scope.appointments);
    $scope.save = function() {
      $scope.appointments.start_at = moment(
        $scope.appointments.start_at
      ).format("YYYY-MM-DD");
      ($scope.appointments.ends_at = moment($scope.appointments.ends_at).format(
        "YYYY-MM-DD"
      )), ($scope.appointments.accomplished_at = moment(
        $scope.appointments.accomplished_at
      ).format("YYYY-MM-DD")), ($scope.appointments.accepted_at = moment(
        $scope.appointments.accepted_at
      ).format("YYYY-MM-DD")), console.log($scope.appointments);

      AgendaAppointmentSrv.save(
        $scope.appointments,
        function(response) {
  ...
  /**
     ...
     * @constructor
     */
  function AgendaAppointmentsEditCtrl(
    $scope,
    $window,
    $stateParams,
    moment,
    ...
    $scope.appointments.accomplished_at = new Date(
      $scope.appointments.accomplished_at
    );
    ...
    $scope.save = function() {
      ...
      ($scope.appointments.ends_at = moment($scope.appointments.ends_at).format(
        ...
        $scope.appointments.accepted_at
      ).format("YYYY-MM-DD")), AgendaAppointmentSrv.save(
        $scope.appointments,
        function(response) {
          console.log(response);
          ToastService.success(response.message);
